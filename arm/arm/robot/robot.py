...
class Robot(abc.ABC):
    ...
    def set_tool(self, translation: np.ndarray, rotation: np.ndarray = None):
        """
        兼容旧接口设置工具坐标系
        :param translation: [x, y, z] 平移
        :param rotation: 可选旋转矩阵或欧拉角
        """
        if rotation is None:
            self._tool = SE3(translation)
        else:
            self._tool = SE3(rotation, translation)
    ...
    def set_base(self, position: np.ndarray):
        """兼容旧接口的方法"""
        self._base = SE3(position)
        self.robot.base = self._base
